  ...
  hakyllWith
    ...
    $ do
      ...
      create ["index.html"] $ do
        route idRoute
        let fragments = ["news", "pubs", "talks", "drafts", "theses", "notes", "teaching", "about"]
        compile $ do
          bodies <- mapM (loadBody . fromString . (++ ".markdown")) fragments
          let fields = zipWith constField fragments bodies
          let indexCtx =
                defaultContext
                  <> mconcat fields
                  <> constField "email1" email1
...
email1 = user <> "@" <> host
  where
    name = map toLower $ head $ words author
    user = "me"
    host = name ++ ".org"
...
email2 = user <> "@" <> host
  where
    name = (map toLower $ head $ words author) ++ "." ++ (map toLower $ head . tail $ words author)
    user = name
    host = "unibo.it"
...
gravatar = "https://www.gravatar.com/avatar/" <> hash <> ext <> size
  where
    hash = md5s . Str $ map toLower email
    ext = ".png"
    size = "?s=192"
